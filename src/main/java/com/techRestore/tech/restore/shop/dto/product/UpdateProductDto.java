...
public record UpdateProductDto(
        @Size(min = 1, message = "Product name cannot be empty if provided") String name,

        @Size(min = 1, message = "Product description cannot be empty if provided") String description,

        @DecimalMin(value = "0.0", inclusive = false, message = "Price must be greater than 0") BigDecimal price,

        @URL(message = "Invalid URL format") String imageUrl,
